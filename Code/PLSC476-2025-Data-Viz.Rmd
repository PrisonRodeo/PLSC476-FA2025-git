---
title: 'PLSC 476: Empirical Legal Studies'
...
# it if it is not, and then (c) loads each of them. BUT...
#
# Before you can do that, you need to run this little bit
# of code, to get the "urbanmapr" package from their
# github repo:

install.packages("devtools")
devtools::install_github("UrbanInstitute/urbnmapr")

# Next, the package code:

P<-c("readxl","visdat","sf","psych","dplyr","ggplot2",
     "tidyr","stringr","lubridate","maps","usmap",
     "urbnmapr","car","lattice","curl")
...
# Run lines 40-50 ten or twelve times until you get all smileys. :)
#
...
- [RMarkdown](https://rmarkdown.rstudio.com/) is "an authoring framework for data science" that facilitates literate programming and reproduceable research
 ...
 - These slides were created using RMarkdown; the code for doing so is available on the course [github repository](https://github.com/PrisonRodeo/PLSC476-FA2025-git/tree/master/Code)
...
# Pull the COVID-19 data directly from the PA Dept. of Health
# website:

cases <- read.csv("https://data.pa.gov/api/views/j72v-r42c/rows.csv?accessType=DOWNLOAD",
                  stringsAsFactors=FALSE) # read the data

# Next, grab the COVID deaths data:

deaths <- read.csv("https://data.pa.gov/api/views/fbgu-sqgp/rows.csv?accessType=DOWNLOAD",
                   stringsAsFactors=FALSE) # read the data
...
# Subset to July 22, 2021 data only (change the date
# if you want to update things):

COVID<-df[df$Date=="07/22/2021",]
...
knitr::kable(COVID[1:12,1:6],caption="PA COVID-19 Data, 07/22/2021")
```
...
par(mar=c(4,4,2,2)) # fix figure margins a bit
with(COVID, dotchart(New.Cases,pch=19,labels=County,
             cex=0.30,xlab="Number of New Cases"))
abline(v=c(20,40,60,80,100),lty=2,lwd=1) # add ref. lines
abline(v=mean(COVID$New.Cases,na.rm=TRUE),lty=1,col="blue")
abline(v=median(COVID$New.Cases,na.rm=TRUE),lty=1,col="orange")
legend("bottomright",bty="n",lty=1,col=c("blue","orange"),
...
```{r fig.cap="Daily COVID Cases in Centre County, 3/1/2020-6/15/2023",fig.width=4.5, fig.asp=0.9,comment=NA} 
...
with(Centre, plot(Date,New.Cases,t="l",
                cex.lab=0.7,cex.axis=0.7))
with(Centre, lines(lowess(Date,New.Cases,f=0.01,
                          delta=20),
                   lwd=2,col="red"))
...
```{r fig.cap="Total Cases (Percent of the Population)",fig.width=4.5, fig.asp=0.9,comment=NA,warning=FALSE,message=FALSE}
...
url <- "https://ers.usda.gov/sites/default/files/_laserfiche/DataFiles/53251/ruralurbancodes2013.xls?v=66529"
destfile <- "UR.xls"
...
- Optimize {readability, information density}
- Each plot should "stand on its own"
- Consider colorblindness (see, e.g., [here](https://www.nceas.ucsb.edu/sites/default/files/2022-06/Colorblind%20Safe%20Color%20Schemes.pdf))
